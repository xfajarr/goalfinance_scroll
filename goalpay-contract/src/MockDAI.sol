...
/**
 * @title MockDAI
 * @notice Mock DAI token for testing and demonstration
 * @dev ERC20 token with 18 decimals to match real DAI
 */
contract MockDAI is ERC20, Ownable {
    uint8 private _decimals = 18;
    
    // Faucet settings
    uint256 public constant FAUCET_AMOUNT = 1000 * 10**18; // 1000 DAI
    ...
    error MockDAI__FaucetCooldown();
    error MockDAI__InvalidAmount();
    
    constructor(uint256 _initialSupply) ERC20("Mock Dai Stablecoin", "DAI") Ownable(msg.sender) {
    ...
    function mint(address _to, uint256 _amount) external onlyOwner {
        if (_amount == 0) revert MockDAI__InvalidAmount();
        _mint(_to, _amount);
    }
