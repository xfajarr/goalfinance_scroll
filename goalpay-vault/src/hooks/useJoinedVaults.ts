...
export function useJoinedVaults(): UseJoinedVaultsReturn {
  ...
  const joinedVaults: VaultData[] = useMemo(() => {
    ...
    return (vaultIds as bigint[]).slice(0, 10).map((id, index) => {
      ...
      return {
        id,
        name: vaultInfo.name || `Vault ${id}`,
        description: vaultInfo.description || '',
        creator: vaultInfo.creator || '0x0',
        token: vaultInfo.token || '0x0',
        goalType: vaultInfo.goalType || 0,
        visibility: vaultInfo.visibility || 0,
        targetAmount: vaultInfo.targetAmount || 0n,
        totalDeposited: vaultInfo.totalDeposited || 0n,
        deadline: vaultInfo.deadline || 0n,
