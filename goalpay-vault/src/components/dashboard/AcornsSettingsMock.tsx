...
interface AcornsSettingsMockProps {
  onClose: () => void;
}
...
const PORTFOLIO_OPTIONS = [
  {
    ...
    icon: Shield,
    details: 'Focus on stable investments like bonds and dividend stocks',
  },
  {
    ...
    icon: TrendingUp,
    details: 'Mix of stocks and bonds for balanced growth',
  },
  {
    ...
    icon: Zap,
    details: 'Growth-focused with higher stock allocation',
  },
];

export const AcornsSettingsMock: React.FC<AcornsSettingsMockProps> = ({ onClose }) => {
  const { 
    userAccount, 
    stats, 
    changePortfolio, 
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Settings className="w-5 h-5" />
            Acorns Settings & Portfolio Management
          </DialogTitle>
        </DialogHeader>
        ...
        <Tabs defaultValue="portfolio" className="w-full">
          ...
          <TabsContent value="portfolio" className="space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Choose your investment strategy based on your risk tolerance and financial goals
              </p>
            </div>
            ...
            {selectedPortfolio !== userAccount.portfolio && (
              <div className="flex gap-3">
                ...
                <Button 
                  variant="outline"
                  onClick={() => setSelectedPortfolio(userAccount.portfolio)}
                  className="flex-1"
                >
                  Cancel
                </Button>
            ...
            <Card className="p-4 bg-gradient-to-r from-blue-50 to-purple-50">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-medium text-blue-800">Performance Projection</span>
              </div>
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div className="text-center">
                  <p className="text-lg font-bold text-blue-600">
                    {formatCurrency(stats?.totalInvested * 1.02 || 0)}
                  </p>
                  <p className="text-gray-600">1 Month</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-blue-600">
                    {formatCurrency(stats?.totalInvested * 1.06 || 0)}
                  </p>
                  <p className="text-gray-600">6 Months</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-blue-600">
                    {formatCurrency(stats?.totalInvested * 1.12 || 0)}
                  </p>
          ...
          <TabsContent value="recurring" className="space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Set up automatic investments to grow your portfolio consistently over time
              </p>
            </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="text-green-700">
                      <strong>{formatCurrency(userAccount.recurringAmount)}</strong> every{' '}
                      <strong>{userAccount.recurringInterval} days</strong>
                    </p>
                  </div>
                  <div>
                    <p className="text-green-700">
                      Next: <strong>{formatDate(userAccount.nextRecurringDate)}</strong>
                    </p>
                  </div>
                </div>
              </Card>
            ...
            {/* Recurring Investment Benefits */}
            <Card className="p-4 bg-blue-50 border-blue-200">
              <div className="flex items-center gap-2 mb-2">
                <TrendingUp className="w-5 h-5 text-blue-600" />
                <span className="font-medium text-blue-800">Benefits of Recurring Investment</span>
              </div>
              <ul className="text-sm text-blue-700 space-y-1">
                <li>• Dollar-cost averaging reduces market volatility impact</li>
                <li>• Consistent investing builds wealth over time</li>
                <li>• Automated approach removes emotional decision-making</li>
                <li>• Compound growth accelerates with regular contributions</li>
              </ul>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="actions" className="space-y-4">
            ...
            <Card className="p-4 border-blue-200 bg-blue-50">
              ...
              <div className="text-sm text-blue-700 space-y-1">
                ...
                <p>Account Age: <strong>2 weeks</strong></p>
              </div>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="history" className="space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Track your spending and round-up investments
              </p>
            </div>
              ...
              {purchases?.slice(0, 10).map((purchase) => (
                <Card key={purchase.id} className="p-3">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="font-medium text-sm">{purchase.merchant}</h4>
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-600">
                        <span>Purchase: {formatCurrency(purchase.amount)}</span>
                        <span>Round-up: {formatCurrency(purchase.roundUpAmount)}</span>
                        <span>{formatDate(purchase.timestamp)}</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium">
                        {formatCurrency(purchase.roundUpAmount)}
                      </p>
                      <p className="text-xs text-gray-500">
                        {purchase.invested ? 'Invested' : 'Pending'}
                      </p>
                    </div>
                  </div>
                </Card>
            ...
            <Card className="p-4 bg-gradient-to-r from-purple-50 to-pink-50">
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-lg font-bold text-purple-600">{purchases?.length || 0}</p>
                  <p className="text-sm text-gray-600">Total Purchases</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-purple-600">
                    {formatCurrency(purchases?.reduce((sum, p) => sum + p.amount, 0) || 0)}
                  </p>
                  <p className="text-sm text-gray-600">Total Spent</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-purple-600">
                    {formatCurrency(purchases?.reduce((sum, p) => sum + p.roundUpAmount, 0) || 0)}
                  </p>
